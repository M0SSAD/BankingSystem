...
void runHardTest() {
    ...
    assert(myBank.withdraw(acc1, 100) == TransactionStatus::FAILED_INVALID_STATE);
    std::cout << "[PASS] State Machine: Blocked withdrawal on Pending account." << std::endl;

    // 2. TEST: Minimum Deposit Rule
    // You defined MINIMUM_PENDING_DEPOSIT = 500
    assert(myBank.deposit(acc1, 100) == TransactionStatus::FAILED_INVALID_STATE);
    assert(myBank.deposit(acc1, 600) == TransactionStatus::SUCCESS);
    // Now account should be ACTIVE
    std::cout << "[PASS] Logic: Enforced minimum deposit to activate account." << std::endl;
    ...
    assert(myBank.transfer(acc2, acc1, 100) == TransactionStatus::FAILED_ACCOUNT_FROZEN);
    std::cout << "[PASS] Security: Prevented transfer from Frozen account." << std::endl;
    ...
    std::cout << "[PASS] Concurrency: Mutexes prevented data corruption under load." << std::endl;

    // 5. TEST: Transaction Indexing
    // Query how many transfers happened from acc1
    auto history = myBank.queryBySrc(acc1);
    assert(history.size() > 0);
    std::cout << "[PASS] Data: Transaction history indexed correctly." << std::endl;
