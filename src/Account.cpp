...
Account::Account(uint64_t account_id, uint64_t customer_id)
    : account_id(account_id), customer_id(customer_id), balance(0),
...
// getters
int64_t Account::getBalance() { return balance; }

AccountState Account::getState() { return state; }

uint64_t Account::getAccountID() { return account_id; }

uint64_t Account::getCustomerID() { return customer_id; }
